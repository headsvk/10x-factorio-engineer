...
| `dev/test_quality_planner.py` | `unittest` suite (164 tests) for quality_planner |
...
### `dev/test_quality_planner.py` (164 tests)
...
| `TestEnableShufflesAll` (V3) | `--enable-shuffles all` sentinel activates every applicable shuffle for the chain (LDS in processing-unit chain); target item excluded as primary (no quantum-processor shuffle when target is processing-unit); iron-plate chain activates zero shuffles (no overlap); unknown shuffle name errors with "unknown shuffle" message; combines cleanly with `--assembly-modules` |
