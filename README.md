...
- **Dashboard mode** — a published `application/vnd.ant.html` artifact with science pack progress bars, bottleneck detection, per-line machine tables, and in-artifact chat. State persists via `window.storage` (cross-device) with `localStorage` fallback. Import/Export buttons sync state with CLI sessions.
...
- **Power output** — MW per production line and total for the factory; efficiency module effect on power draw
